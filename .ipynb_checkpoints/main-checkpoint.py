...
# 用于可视化节点梯度的函数
def visualize_node_gradients(graph, gradients):
    G = to_networkx(graph)  # 将PyTorch Geometric的图转为networkx图
    pos = nx.spring_layout(G)  # 节点布局
    
    # 归一化梯度值以便用于颜色映射
    norm = plt.Normalize(vmin=gradients.min().item(), vmax=gradients.max().item())
    
    # 使用matplotlib的colormap
    node_colors = [cm.viridis(norm(grad.item())) for grad in gradients]
    
    plt.figure(figsize=(8, 8))
    
    # 绘制网络图，节点颜色根据梯度大小变化
    nx.draw(G, pos, with_labels=True, node_color=node_colors, node_size=500, cmap=plt.cm.viridis)
...
def train(model, device, loader, optimizer_gnn, optimizer_seg, args):
    ...
    for step, batch in enumerate(loader):
        ...
        # 获取节点梯度
        gradients = batch.x.grad
        
        # 可视化梯度权重
        visualize_node_gradients(batch, gradients)
        optimizer_gnn.step()
...
def eval(model, device, loader):
    ...
    for step, batch in enumerate(loader):
        ...
        #     degrees = [degree.item() for degree in data.c2]
        #     graph_sizes.append(degrees)
        # graph_sizes.append([data.num_nodes for data in batch.to_data_list()])
    ...
    return {'acc': correct.sum().item()/correct.shape[0], 'sizes': graph_sizes, 'y_true': y_true.cpu(), 'y_pred': y_pred.cpu()}
...
def main(args):
    ...
    with open('./asreadout.csv','a+')as f:
        f.write('{}\t{:.4f}\t{}\t{}\t{}\t{}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\n'.format(
            args.gnn, args.drop, args.batch_size, args.score_nums, args.relu, args.dataset, np.mean(valid_list), np.std(valid_list), np.mean(test_list), np.std(test_list))
        )
